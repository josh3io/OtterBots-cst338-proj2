...
public class TestLiveDataObserver<T> {
   ...
   * @param condition Wait for this condition to be true to run our change handler. this protects
   *                  against early intermediate changes
   * @param handler   This is what will run on our LiveData wrapped data. we want to do our data
   *                  testing here.
   ...
   */
  boolean test(LiveData<T> liveData, java.util.function.Predicate<T> condition,
      LiveDataOnChangedHandler<T> handler) throws InterruptedException {
    return test(liveData, condition, handler, LATCH_TIMEOUT);
  }

  boolean test(LiveData<T> liveData, Predicate<T> condition, LiveDataOnChangedHandler<T> handler,
      Integer timeoutSeconds) throws InterruptedException {
   ...
   * @throws TimeoutException timeout trying to get the data observed that satisfies the predicate
   *                          condition
   */
   ...
   *                          condition
   */
  T getOrAwaitValue(LiveData<T> liveData, Predicate<T> condition, Integer timeoutSeconds)
